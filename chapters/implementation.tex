...
As Focaccia deals in \textbf{program states} rather than instructions, the only thing necessary to support more
architectures than the currently supported x86 is to define a list of an architecture's registers. Focaccia has a
built-in mechanism to define subsets of registers (e.g. \texttt{EAX = RAX[0:32]}) as well as state flags. Peripheral
tools such as communication with \texttt{LLDB} or \texttt{GDB} may need to be implemented for specific architectures as
well, depending on the consistency of those tools' interfaces.
